...
-- locação: update_rental edita equipamento/data/disparos/valor/forma/
-- status, mas não tem parâmetro de cliente, e a tela de evento da
-- Agenda bloqueia totalmente a edição quando o evento veio de uma
-- locação, sobrando só "Ir para a locação" — que também não deixa
-- trocar o cliente. Resultado: uma locação lançada no cliente errado
-- (ex: confundiu duas pessoas na agenda) ficava presa lá para sempre,
-- sem opção de correção, só cancelar e recriar do zero.
--
